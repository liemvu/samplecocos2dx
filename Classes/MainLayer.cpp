...
CCScene* MainLayer::scene()
{
    ...
    ccNodeLoaderLibrary->registerCCNodeLoader("CCParallaxNodeExtras", CCParallaxNodeExtrasLoader::loader());
    cocos2d::extension::CCBReader * ccbReader = new cocos2d::extension::CCBReader(ccNodeLoaderLibrary);
    CCScene *pScene = ccbReader->createSceneWithNodeGraphFromFile("MainLayer");
    ccbReader->release();
...
bool MainLayer::onAssignCCBMemberVariable(cocos2d::CCObject * pTarget, const char * pMemberVariableName, cocos2d::CCNode * pNode) {
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "_ship", CCSprite *, _ship);
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "_backgroundNode", CCParallaxNodeExtras *, _backgroundNode);
...
void MainLayer::setShipTargetPos(CCPoint location) {
    CCSize winSize = this->getScreenSize();//CCDirector::sharedDirector()->getWinSize();
...
void MainLayer::shoot() {    
    ...
    shipLaser->runAction(CCSequence::create(
                                            ...
                                            CCCallFuncN::create(this, callfuncN_selector(MainLayer::setInvisible)),
                                            NULL));
...
void MainLayer::endScene( EndReason endReason ) {
    if (_gameOver)
        return;
    _gameOver = true;
    ...
    CCMenuItemLabel *restartItem = CCMenuItemLabel::create(restartLabel, this, menu_selector(MainLayer::restartTapped) );
...
float MainLayer::randomValueBetween(float low, float high) {
    return (((float) arc4random() / 0xFFFFFFFFu) * (high - low)) + low;
}

float MainLayer::getTimeTick() {
    timeval time;
...
void MainLayer::setInvisible(CCNode * node) {
    node->setVisible(false);
}

CCSize MainLayer::getScreenSize() {
